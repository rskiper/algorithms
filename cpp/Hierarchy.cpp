...
int main()
{
	...
	if (n < m)	//it doesn't matter who is sender and recipient, distance is the same, so let sender will be less than recipient
	{
		unsigned int r = n;
		n = m;
		m = r;
	...
	unsigned int days = 0;	//count for days needed to receive message from m to n
	unsigned power = 2;	//count for power of 2
	unsigned e2 = 4;	//2 powered to power value
	//Iterate over powers of 2 and count the number of nodes that are multiples of it, between m and n
	while (e2 <= n)
	{
		unsigned first = 0;	//first node which is multiple of e2
		unsigned last = 0;	//last node which is multiple of e2
		unsigned i;
		e2 = (int)pow(2, power);
		...
		{					//Count for amount of nodes which is multiple exactly of this power of 2 and located strictly within the interval (m; n)
			unsigned k = (last - first) / (2 * e2) + 1;	//number of nodes which is multiple of e2 power of 2
